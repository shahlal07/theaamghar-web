...
function LoginForm() {
  ...
  return (
    <AuthSplitLayout
      ...
      subhead="Sign in to track orders, manage your wishlist, and pick up right where you left off — across the Nashemann marketplace."
    >
      <h1 className="font-serif text-3xl font-bold text-center mb-2">Welcome Back</h1>
      <p className="text-ink-light text-center mb-6">Sign in to your Nashemann account</p>
        ...
        <>
          ...
          <form action={formAction} className="flex flex-col gap-4">
            ...
            <div>
              ...
              <input id="email" name="email" type="email" required className="w-full border border-border-subtle rounded-brand-sm px-4 py-3 text-sm" />
            </div>
            <div>
              <div className="flex items-center justify-between mb-1">
                <label htmlFor="password" className="text-sm font-medium">Password</label>
                <Link href={`/forgot-password?returnTo=${encodeURIComponent(returnTo)}`} className="text-xs font-semibold text-mango-orange">Forgot password?</Link>
              </div>
              <input id="password" name="password" type="password" required className="w-full border border-border-subtle rounded-brand-sm px-4 py-3 text-sm" />
            </div>
            {state?.error && (
              <p className="text-sm text-error">
                {state.error}{" "}
                <Link href={`/forgot-password?returnTo=${encodeURIComponent(returnTo)}`} className="font-semibold underline">Forgot password?</Link>
              </p>
            )}
            <button type="submit" disabled={pending} className="bg-mango-orange text-white font-semibold py-3.5 rounded-full disabled:opacity-60">
