...
export function Testimonials({ reviews }: { reviews: TestimonialReview[] }) {
  ...
  return (
    <Section aria-label="Customer reviews" className="bg-gradient-to-b from-[#FFF9F2] to-[#FBEFDC]">
          ...
          return (
            ...
            >
              <GlassCard className="p-7 h-full flex flex-col">
                <div className="flex gap-0.5 text-[#F4B400] mb-3" aria-hidden="true">
                ...
                {review.title && (
                  <div className="font-serif font-bold text-lg text-[#4A2C12] mb-1">
                    {review.title}
                  </div>
                )}
                <p className="text-[#2D2D2D]/80 leading-relaxed flex-1">&ldquo;{review.body}&rdquo;</p>
                <div className="mt-5 pt-4 border-t border-[#4A2C12]/10 text-sm">
                  <span className="font-semibold text-[#4A2C12]">
                    {profile?.name ?? "Verified Customer"}
                  </span>
                  {product && <span className="text-[#2D2D2D]/60"> · {product.name}</span>}
                </div>
              </GlassCard>
            </motion.div>
          );
