...
## Critical: this Supabase project is shared with `../vendor-admins`

The database (`eznxsosvsgkhexbjoolh`, name `theaamghar`) is **not exclusive to this repo**. A separate, much more mature Next.js app at `E:\Claude\vendor-admins` (14 of 17 planned phases complete: inventory, coupons, analytics, reports, notifications, RLS-hardened) reads and writes the same tables and has extended the schema well beyond what this repo originally created — see its own `CLAUDE.md` for the full picture, especially the "Data conventions" section. Concretely, this means:
...
- **Do not build another admin dashboard here.** That's what `vendor-admins` is for; this repo is the customer-facing storefront only.
